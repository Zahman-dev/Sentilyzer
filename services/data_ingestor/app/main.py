...
class DataIngestor:
    ...
    def save_articles(self, articles: List[Dict[str, Any]]) -> int:
        ...
        for article_data in articles:
            try:
                ...
                existing = self.session.query(RawArticle).filter(
                    RawArticle.article_url == article_data['article_url']
                ).first()
                ...
                logger.error(f"Error saving article {article_data.get('article_url', 'unknown')}: {str(e)}")
                self.stats["errors"] += 1
    ...
    async def collect_all_feeds(self):
        ...
        if self.stats["total_saved"] % 100 == 0 and self.stats["total_saved"] > 0:
            ...
            logger.info(f"Ingestion stats: {self.stats['total_saved']} saved, "
