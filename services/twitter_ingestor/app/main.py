...
class TwitterIngestor:
    ...
    def _generate_mock_tweets(self, search_term: str, count: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        for i in range(count):
            tweet_id = f"mock_tweet_{search_term.replace(' ', '_')}_{i}_{int(time.time())}"
            mock_tweet = {
                'id': tweet_id,
    ...
    def fetch_tweets(self, search_term: str, max_results: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def process_tweet_to_article(self, tweet: Dict[str, Any], search_term: str) -> Dict[str, Any]:
        """
        ...
        with get_database_session() as db:
            for article_data in articles:
                try:
                    ...
                    db.commit()
                    saved_count += 1
                    logger.info(f"Saved tweet article: {article_data['headline'][:50]}...")
                    
                except IntegrityError:
                    # Article already exists (duplicate URL)
                    db.rollback()
                    logger.debug(f"Duplicate article skipped: {article_data['article_url']}")
    ...
    async def collect_and_process_tweets(self):
        ...
        for search_term in self.search_terms:
            try:
                ...
                logger.info(f"Processed {len(tweets)} tweets, saved {saved_count} new articles for '{search_term}'")
        ...
        logger.info(f"Twitter collection completed. Total new articles: {total_processed}")
