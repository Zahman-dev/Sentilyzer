...
class TestPhase3Features:
    ...
    def test_dashboard_accessibility(self, dashboard_url):
        ...
        try:
            ...
            assert "streamlit" in response.text.lower() or "sentilyzer" in response.text.lower()
    ...
    def test_twitter_ingestor_data_format(self):
        ...
        with create_db_session() as session:
            ...
            twitter_articles = session.query(RawArticle).filter(
                RawArticle.source == 'twitter'
            ).limit(5).all()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data["data"]:
                    ...
                    required_fields = [
                        "article_url", "headline", "published_at", 
    ...
    def test_data_pipeline_integration(self):
        ...
        with create_db_session() as session:
            ...
            sentiment_scores = session.query(SentimentScore).filter(
                SentimentScore.article_id == processed_article.id
            ).all()
    ...
    def test_system_statistics_and_health(self):
        ...
        with create_db_session() as session:
            ...
            processed_articles = session.query(RawArticle).filter(
                RawArticle.is_processed == True
            ).count()
            
            # Count error articles
            error_articles = session.query(RawArticle).filter(
            ...
            error_rate = (error_articles / total_articles * 100) if total_articles > 0 else 0
